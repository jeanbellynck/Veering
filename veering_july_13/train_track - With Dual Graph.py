...
class TrainTrack:
    ...
    def print_traintrack(self):
        ...
        for branch in self.branches.values():
            print branch
        self.print_dualgraph()

    def print_dualgraph(self):
        # Initialize adjacency matrix
        adj = [["" for i in range(self.no_of_puncs)] for j in range(self.no_of_puncs)]
        for branch in self.branches.values():
            adj[branch.puncs[0]][branch.puncs[1]] += str(branch.n) + " "
            adj[branch.puncs[1]][branch.puncs[0]] += str(-branch.n) + " "
        # FInd longest entry in each column
        col_widths = [0 for i in range(self.no_of_puncs)]
        for i in range(self.no_of_puncs):
            for j in range(self.no_of_puncs):
                col_widths[j] = max(col_widths[j], len(adj[i][j]))

        # Print adjacency matrix such that columns align
        print "Dual graph:"
        for i in range(self.no_of_puncs):
            for j in range(self.no_of_puncs):
                print "[" + adj[i][j].rjust(col_widths[j]) + "] ",
            print ""
